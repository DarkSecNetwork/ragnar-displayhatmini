...
PY
  echo "Adding early 'Loading Ragnar...' draw so display is not black during startup..."
  python3 <<PY
from pathlib import Path
display_py = Path("$RAGNAR_DIR/display.py")
if display_py.exists():
    text = display_py.read_text()
    old = """        # Wait for deferred initialization (fonts, images) to finish
        # before attempting to render anything.
        if hasattr(self.shared_data, 'wait_for_deferred_init'):"""
    new = """        # Early draw so display is not black during deferred init (Display HAT Mini)
        try:
    ...
    if old in text and new not in text:
        text = text.replace(old, new)
        display_py.write_text(text)
        print("  Patched display.py: early Loading Ragnar draw")
    else:
        print("  display.py already patched or format changed")
PY
fi

# Boot splash for Display HAT Mini: show "Booting...", "Starting Ragnar...", "Loading..." before main process
if [ "$DISPLAY_MODE" = "displayhatmini" ]; then
